...
Then('{string} をクリックする', cy.clickText )
...
And('{string} ボタンをクリックする', (text) => cy.clickTextIn(text, 'button') )
...
Then('/^(?:|ユーザーは )ホームページを表示している$/', cy.visit)
// Then('/^(?:|ユーザーは )ホームページへ移動する$/', )
// Then('/^(?:|ユーザーは )"(?P<page>[^\s]+)" を表示している$/u', )
// Then('/^(?:|ユーザーは )"(?P<page>[^\s]+)" を表示している$/u', cy.visit)
Then('{string} を表示している', cy.visit)
...
Then('{string} フィールドに {string} と入力する', (selector, value) => cy.typeStringInto(value, selector) )
...
Then('I should see the following error messages: {string}', cy.stringExist )
Then('{string} を含むエラーメッセージが表示されるはずです', cy.stringExist )
...
Then('I should see the following success messages: {string}', cy.stringExist )
Then('{string} の成功メッセージが表示されるはずです', cy.stringExist )
